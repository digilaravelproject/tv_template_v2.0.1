...
function tvApp() {
    return {
        ...
        // --- APPLICATIONS (OTT APPS) CONTROLLER ---
        getActiveOttList() {
        ...
        getAppBrandBadgeClass(app, isActive) {
            const id = (app.id || '').toLowerCase();
            if (id.includes('sony')) return 'bg-gradient-to-br from-blue-700 to-indigo-950 border-blue-400/50 text-white';
            if (id.includes('jio')) return 'bg-gradient-to-br from-pink-600 to-purple-950 border-pink-400/50 text-white';
            if (id.includes('aha')) return 'bg-gradient-to-br from-orange-600 to-red-950 border-orange-400/50 text-white';
            if (id.includes('sun')) return 'bg-gradient-to-br from-red-600 to-amber-950 border-red-400/50 text-white';
            if (id.includes('mx')) return 'bg-gradient-to-br from-blue-600 to-cyan-950 border-cyan-400/50 text-white';
            if (id.includes('discovery')) return 'bg-gradient-to-br from-emerald-600 to-teal-950 border-emerald-400/50 text-white';
            if (id.includes('alt')) return 'bg-gradient-to-br from-rose-600 to-red-950 border-rose-400/50 text-white';
            if (id.includes('eros')) return 'bg-gradient-to-br from-fuchsia-600 to-purple-950 border-fuchsia-400/50 text-white';
            if (id.includes('hungama')) return 'bg-gradient-to-br from-amber-600 to-orange-950 border-amber-400/50 text-white';
            if (id.includes('hoichoi')) return 'bg-gradient-to-br from-red-600 to-rose-950 border-red-400/50 text-white';
            if (id.includes('planet')) return 'bg-gradient-to-br from-amber-600 to-yellow-950 border-yellow-400/50 text-white';
            if (id.includes('chaupal')) return 'bg-gradient-to-br from-amber-700 to-orange-950 border-amber-400/50 text-white';
            if (id.includes('manorama')) return 'bg-gradient-to-br from-blue-700 to-slate-950 border-blue-400/50 text-white';
            if (id.includes('voot')) return 'bg-gradient-to-br from-purple-700 to-violet-950 border-purple-400/50 text-white';
            return 'bg-gradient-to-br from-amber-600/60 to-black border-amber-400/50 text-amber-300';
        },
        ...
        focusCurrentApp() {
            this.$nextTick(() => {
                if (typeof this.activeAppFocusIndex === 'number') {
                    ...
                    document.getElementById('apps-btn-back')?.focus();
                }
            });
        },
